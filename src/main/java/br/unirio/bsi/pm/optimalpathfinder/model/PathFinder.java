...
/**
 * This class implements the A* path finding algorithm to calculate the best path on a track file
 * 
...
public class PathFinder {
	...
	public ArrayList<Coordinate2D> calculatePath(Coordinate2D source, Coordinate2D target){
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					Node candidateNode = new  Node(x + currentNode.x,
										           y + currentNode.y);
					
			    	// Off boundaries check
			    	if ((candidateNode.x > 0) && 
				    	(candidateNode.y > 0) &&
				    	(candidateNode.x < track.getRows()) &&
		...
		// Now backtrack the node map, from target to source, through the node parents
		Node cursorNode = map[target.getX()][target.getY()];
		while ( (cursorNode.x != source.getX()) || 
				(cursorNode.y != source.getY())) 
		{
	...
	private class Node implements Comparable<Object>{
		...
		private int y;
		/** the movement cost (in number of squares) from the start point A to the current square*/
		private int cost;
		/** the estimated movement cost (in number of squares ) from the current square to the destination point */
		private int depth;
